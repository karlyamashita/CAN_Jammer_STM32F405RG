...
void USB_to_CAN_Send(CAN_MsgStruct *msg, uint8_t *data)
{
	...
	CAN_SendMessage(msg); // try sending
}
...
void CAN_to_USB_Send(CAN_MsgStruct *msg, uint8_t node)
{
	...
	if(msg->hcan == &hcan1)
	{
		usbMsgTemp.Status.id = CMD_CAN1_MESSAGE; // maybe we can stick with CMD ID 0x80 for either CAN1 or CAN2,
													// but we can use Node to differentiate CAN1 and CAN2?
		usbCanStruct.msgBytes.Node = CAN1_NODE;
	}
	else if(msg->hcan == &hcan2)
	{
		usbMsgTemp.Status.id = CMD_CAN2_MESSAGE;
		usbCanStruct.msgBytes.Node = CAN2_NODE;
	}
