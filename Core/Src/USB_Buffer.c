...
int USB_SendMessage(USB_MsgStruct *msg)
{
	...
	{
		USB_Status = CDC_Transmit_HS(msg->txQueue[msg->txPtr.index_OUT].Byte.data, ID_AND_SIZE_LENGTH + msg->txQueue[msg->txPtr.index_OUT].Status.size);
...
** Open file USB_DEVICE>App>usbd_cdc_if.c and add USB_AddRxBuffer function call to CDC_Receive_FS as shown below **
...
static int8_t CDC_Receive_FS(uint8_t* Buf, uint32_t *Len)
{
  ...
  USB_AddRxBuffer(&usb_msg, Buf, *Len); // add this function here, assuming you created usb_msg
